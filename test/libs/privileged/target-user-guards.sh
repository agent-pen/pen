...
#
# Provides:
#   ensure_correct_target_user <username>          — name + invoking-user check
#   ensure_correct_target_user_and_uid <username>  — additionally checks UID differs
#   resolve_target_uid <username>           — print UID to stdout
#   ensure_correct_target_path <path>              — path is under test user home
...
ensure_correct_target_user_and_uid() {
  local target="$1"
