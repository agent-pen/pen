...
- **Git commits:** Execute `git add` and `git commit` as separate Bash tool calls (not chained with `&&`), so the user can configure persistent permissions for each command independently.
